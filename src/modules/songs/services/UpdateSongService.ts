...
export class UpdateSongService {
  async execute({ id, title, artist, tone, bpm, youtube_url, spotify_url, chord_pro, time_signature, organization_id }: Request) {
    ...
    const updatedSong = await songsRepository.update({
      ...
      spotify_url: spotify_url !== undefined ? spotify_url : songExists.spotify_url,
