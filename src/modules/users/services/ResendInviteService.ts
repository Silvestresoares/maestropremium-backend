...
export class ResendInviteService {
  ...
  async execute(userId: string | number, organization_id?: string) {
    ...
    if (organization_id) {
      const isMember = await this.usersRepository.checkUserInOrganization(user.id, organization_id);
      if (!isMember) {
        throw new AppError('Usuário não pertence a esta equipe.', 403);
      }
    }
